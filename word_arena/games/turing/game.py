...
class TuringGame(BaseGame[TuringInfo, None, TuringGuess, TuringFeedback, TuringFinalResult]):
    def __init__(
        self, *, code: int, verifiers: list[list[str]], keys: list[int], max_guesses: int
    ) -> None:
    ...
    def start_game(self) -> TuringInfo:
        ...
        return TuringInfo(verifiers=self._verifiers, max_guesses=self._max_guesses)

    @override
    def is_over(self) -> bool:
        return self._final_verdict is not None or self._num_guesses >= self._max_guesses > 0
